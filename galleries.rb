...
# 
# Authors: Matt Harzewski, Oleksii Schastlyvyi, Markus Konrad, Derek Liang
...
module Jekyll
		...
		def gallery_images(context)
			...
			input_data.each do |item|
				...
				if File.directory?(full_item_path) && item_ok   # check if this item points to a directory
					itemnum = 1
					Dir.glob(File.join(full_item_path, "*.{png,jpg,jpeg,gif,JPG}")).each do |file|
	...
	class ThumbGenerator < Generator
		...
		def files_to_resize(site)
			...
			@fileRegex = File.join(@gallery_dir, "**", "*.{png,jpg,jpeg,gif,JPG}")

			Dir.glob(@fileRegex).each do |file|
